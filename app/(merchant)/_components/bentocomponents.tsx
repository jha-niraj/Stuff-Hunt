					...
					</div>
					{
						integrations.map((integration, index) => (
							<div
								...
								className={`flex items-center justify-between p-3 w-full ${index < integrations.length - 1 ? "border-b border-border" : ""
									}`}
							>
						...
						<div key={rowIndex} className="flex justify-start items-center gap-4">
							{
								integrations.slice(rowIndex * 10, (rowIndex + 1) * 10).map((item, colIndex) => (
									<div
										key={colIndex}
										className={`w-14 h-14 relative rounded-lg border border-border flex justify-center items-center overflow-hidden flex-shrink-0 ${item.isActive
